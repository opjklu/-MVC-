    ...
    private static function printError($error)
    {
        ...
        if (APP_DEBUG||IS_CLI){ //是否开启了调试模式 命令行格式
           ...
           // iconv — 字符串按要求的字符编码来转换
            IS_CLI?exit(iconv("UTF-8", "GBK", $e["message"].PHP_EOL."File".$e["file"]."({$e['line']})".PHP_EOL.$e['trace'])) : die("<pre>".$e["trace"]."</pre>");
        }else{
            //获取错误页面
       ...
       $errorShow = getConfig("TMPL_EXCEPTION_FILE" , null , TPL_PATH."think_exception.tpl");
