...
class  View
{
    ...
    protected function getThemePath($file = null ,$model = __MODULE__)
    {
        ...
        if (defined('THEME_PATH')) {
            ...
            define('THEME_PATH', $viewPath ? APP_PATH.$model.'/'.$templateFile :  APP_PATH.$model.'/'.getConfig('DEFAULT_C_LAYER').'/'.$templateFile );
        }
        
        //分析模板规则
        if (null === $templateFile){
            $themeFile = __CONTROLLER__.$depar.__ACTION__;
        } else if (false === strpos($templateFile, $depar)){
            $themeFile = __CONTROLLER__.$depar.$templateFile;
        }
        $thisFile = THEME_PATH.$depar.$themeFile.getConfig('TMPL_TEMPLATE_SUFFIX');
        $thisFile = getConfig('TMPL_LOAD_DEFAULTTHEME') && 
                    is_file($thisFile) && THEME_PATH == getConfig('DEFAULT_THEME') ? 
                    $thisFile : dirname(THEME_PATH).'/'.getConfig('DEFAULT_THEME').'/'.$templateFile.getConfig('TMPL_TEMPLATE_SUFFIX');
        return $thisFile;
    }
    /**
     * 设置当前主题
    ...
    public function parseHtmlAndPHP($templateFile = null, $content = null, $front = null)
    {
        ...
        if (empty($content)) {
            //定位模板主题
            $templateFile = $this->getThemeName();
